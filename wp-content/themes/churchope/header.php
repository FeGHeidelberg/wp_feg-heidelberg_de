...
<!--[if gt IE 8]><!--> <html class="no-js" <?php language_attributes(); ?>> <!--<![endif]-->
	...
	if (is_singular()) {
		...
		if(get_tax_meta($current_term->term_id, SHORTNAME . "_custom_term_header", true) ) {
			$custom_header_settings		= true;
	...
	//var_dump($custom_header_color);
	if(!empty($custom_header_settings)) {
		echo 	"<style>
					#color_header {background:$custom_header_color url('$custom_header_pattern') $custom_header_repeat $custom_header_horizontal $custom_header_vertical; }
				</style>";
	}
	?>
				...
				<div class="header_top">
					<div class="row">
						<div class="logo grid_6">
							<?php if (get_option(SHORTNAME . "_logo_txt") || get_option(SHORTNAME . "_logo_custom") ) { ?>
